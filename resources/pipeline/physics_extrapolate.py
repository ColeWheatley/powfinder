...
Input files (static, same folder):
    ...
    resources/terrains/tirol_25m_float.tif  – elevation look-up
...
DEM_PATH = THIS_DIR.parent / "terrains" / "tirol_5m_float.tif"  # Use 5m DEM for better accuracy
...
def read_dem_elevation(lat, lon, dataset):
    # Transform WGS84 (EPSG:4326) coordinates to the DEM's CRS
    ...
    try:
        ...
        if dataset.nodata is not None and elev == dataset.nodata:
            return None
        # Allow 0 elevation (valleys, sea level areas)
        return float(elev)
    ...
    try:
        ...
        for coord_entry in data['coordinates']:
            if coord_entry['status'] == 'collected' and coord_entry.get('weather_data_3hour'):
                ...
                # Handle elevation conversion, skip entries with unknown elevation
                try:
                    elevation = float(coord_info['elevation'])
                except (ValueError, TypeError):
                    continue  # Skip entries with non-numeric elevation
