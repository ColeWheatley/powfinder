#!/usr/bin/env python3
"""Generate surface pressure GeoTIFFs by interpolating weather data to a
100m grid.

This script reads the aggregated 3-hour weather dataset and creates
interpolated surface pressure rasters for a set of predefined timestamps.
The output structure is::

    TIFS/100m_resolution/<timestamp>/surface_pressure.tif

The interpolation uses up to four nearest weather points with inverse
distance weighting and applies a simple barometric adjustment based on
...
# Color scale configuration for surface pressure range
...
def load_weather_points(path):
    ...
    for entry in entries:
        ...
        pressures.append(np.array(hourly["surface_pressure"], dtype=np.float32))

    return np.array(coords), np.stack(pressures), times
...
def main():
    ...
    coords, pressures, times = load_weather_points(WEATHER_PATH)
    ...
    if os.path.exists(COLOR_SCALE_JSON):
        with open(COLOR_SCALE_JSON, "r", encoding="utf-8") as fp:
            scales = json.load(fp)
        color_scale = scales.get("surface_pressure", {})
        print(
            f"Surface pressure scale range: {color_scale.get('min')} to {color_scale.get('max')} hPa"
        )
    ...
    for ts in TIMESTAMPS:
        ...
        # Hillshade is unused but read for consistency with other scripts
        _ = hillshade[period].filled(0).ravel()
        pressure_adj = apply_pressure_adjustment(p_k, grid_elev_flat[:, None], e_k)

        w = 1.0 / np.maximum(dists, 1e-6) ** 2
        w /= w.sum(axis=1, keepdims=True)

        out_flat = np.sum(pressure_adj * w, axis=1)
        ...
        # Map floating point surface pressure to 0-255 byte range using color scale
        if color_scale and "min" in color_scale and "max" in color_scale:
            pmin = color_scale["min"]
            pmax = color_scale["max"]
        else:
            # fallback to min/max from data
            pmin = float(np.nanmin(out[out != NODATA]))
            pmax = float(np.nanmax(out[out != NODATA]))

        # Clip and scale
        clipped = np.clip(out, pmin, pmax)
        ...
        out_path = os.path.join(out_dir, "surface_pressure.tif")
        ...
        with rasterio.open(out_path, "w", **profile) as dst:
            ...
            dst.update_tags(units="hPa", processing_time=datetime.utcnow().isoformat(),
