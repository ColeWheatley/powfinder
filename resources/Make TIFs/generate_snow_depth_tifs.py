#!/usr/bin/env python3
"""Generate snow depth GeoTIFFs by interpolating weather data to a 100m grid."""
...
def load_weather_points(path):
    ...
    for entry in entries:
        ...
        depths.append(np.array(hourly["snow_depth"], dtype=np.float32))

    return np.array(coords), np.stack(depths), times
...
def main():
    ...
    coords, depths, times = load_weather_points(WEATHER_PATH)
    ...
    if os.path.exists(COLOR_SCALE_JSON):
        with open(COLOR_SCALE_JSON, "r", encoding="utf-8") as fp:
            scales = json.load(fp)
        color_scale = scales.get("snow_depth", {})
    ...
    for ts in TIMESTAMPS:
        ...
        out_path = os.path.join(out_dir, "snow_depth.tif")
        if os.path.exists(out_path):
            os.remove(out_path)
        with rasterio.open(out_path, "w", **profile) as dst:
            dst.write(out_byte, 1)
            dst.update_tags(units="m", processing_time=datetime.utcnow().isoformat(),
