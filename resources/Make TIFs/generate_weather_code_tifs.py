#!/usr/bin/env python3
"""Generate Weather Code GeoTIFFs by interpolating weather data to a 100m grid.

This script mirrors ``generate_t2m_tifs.py`` but outputs WMO ``weather_code``
values instead of temperature.  The timestamps, resolution and colour mapping
match the temperature workflow so the resulting files integrate seamlessly with
the frontend.  Weather codes are discrete so no lapse-rate or hillshade
adjustments are applied.

Output structure::

    TIFS/100m_resolution/<timestamp>/weather_code.tif
"""
...
# Color scale configuration for discrete weather codes
COLOR_SCALE_JSON = "color_scales.json"
OUTPUT_BASE = "TIFS/100m_resolution"
NODATA = -9999.0

# Weather-code interpolation does not use hillshade, but we keep the same
# timestamp list for compatibility with the t2m workflow.
...
def load_weather_points(path):
    ...
    for entry in entries:
        ...
        codes.append(np.array(hourly["weather_code"], dtype=np.float32))

    return np.array(coords), np.stack(codes), times
...
def apply_physics(code, target_elev, source_elev):
    """Return weather code unchanged (no physics adjustments)."""
    return code
...
def main():
    ...
    coords, codes, times = load_weather_points(WEATHER_PATH)
    ...
    if os.path.exists(COLOR_SCALE_JSON):
        with open(COLOR_SCALE_JSON, "r", encoding="utf-8") as fp:
            scales = json.load(fp)
        color_scale = scales.get("weather_code", {})
        if color_scale:
            print(
                f"Weather code scale range: {color_scale.get('min')} to {color_scale.get('max')}"
            )
    ...
    for ts in TIMESTAMPS:
        ...
        out_path = os.path.join(out_dir, "weather_code.tif")
        ...
        with rasterio.open(out_path, "w", **profile) as dst:
            ...
            dst.update_tags(units="wmo_code", processing_time=datetime.utcnow().isoformat(),
