#!/usr/bin/env python3
"""Generate shortwave radiation GeoTIFFs by interpolating weather data to a
100m grid.
...
# Color scale configuration for radiation range
...
def main():
    ...
    if os.path.exists(COLOR_SCALE_JSON):
        with open(COLOR_SCALE_JSON, "r", encoding="utf-8") as fp:
            scales = json.load(fp)
        color_scale = scales.get("shortwave_radiation", {})
        print(
            f"Radiation scale range: {color_scale.get('min')} to {color_scale.get('max')} W/m^2"
        )
    ...
    for ts in TIMESTAMPS:
        ...
        out_flat = np.sum(phys_adj * w, axis=1)
        ...
        # Map floating point radiation to 0-255 byte range using color scale
        ...
        with rasterio.open(out_path, "w", **profile) as dst:
            ...
            dst.update_tags(units="W/m^2", processing_time=datetime.utcnow().isoformat(),
