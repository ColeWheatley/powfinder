#!/usr/bin/env python3
"""Generate Skiability GeoTIFFs based on SQH, wind and weather code."""
...
def load_weather_points(path):
    ...
    for entry in entries:
        ...
        depth.append(np.array(hourly["snow_depth"], dtype=np.float32))
        wind.append(np.array(hourly["wind_speed_10m"], dtype=np.float32))
        codes.append(np.array(hourly["weather_code"], dtype=np.float32))

    return np.array(coords), np.stack(depth), np.stack(wind), np.stack(codes), times
...
def main():
    ...
    coords, depth, wind, codes, times = load_weather_points(WEATHER_PATH)
    ...
    if os.path.exists(COLOR_SCALE_JSON):
        with open(COLOR_SCALE_JSON, "r", encoding="utf-8") as fp:
            scales = json.load(fp)
        color_scale = scales.get("skiability", {})
    ...
    for ts in TIMESTAMPS:
        ...
        sqh = np.sum(v_k * w, axis=1)
        wind_adj = np.sum(w_k * w, axis=1)
        codes_adj = np.sum(penalties * w, axis=1)
        ski = sqh - 0.1 * wind_adj - codes_adj
        ski = np.clip(ski, 0, None)
        ski = np.where(grid_elev_flat == NODATA, NODATA, ski)
        out = ski.reshape(grid_elev.shape).astype(np.float32)
        ...
        out_path = os.path.join(out_dir, "skiability.tif")
        if os.path.exists(out_path):
            os.remove(out_path)
        with rasterio.open(out_path, "w", **profile) as dst:
            dst.write(out_byte, 1)
            dst.update_tags(units="ski", processing_time=datetime.utcnow().isoformat(),
