#!/usr/bin/env python3
"""Generate SQH (Snow Quality Heuristic) GeoTIFFs using snow depth as a proxy."""
...
def load_weather_points(path):
    ...
    return np.array(coords), np.stack(depth), times
...
def main():
    ...
    coords, depth, times = load_weather_points(WEATHER_PATH)
    ...
    for ts in TIMESTAMPS:
        ...
        out_path = os.path.join(out_dir, "sqh.tif")
        if os.path.exists(out_path):
            os.remove(out_path)
        with rasterio.open(out_path, "w", **profile) as dst:
            dst.write(out_byte, 1)
            dst.update_tags(units="sqh", processing_time=datetime.utcnow().isoformat(),
